...
def cross_validation(model, X, y, nFolds):
    ...
    accuracy_scores = [0 for l in range(nFolds)]

    for i in range(nFolds):
        # TODO: Generate indices of samples for the validation set for the fold
        valid_indices = [n + fold_size * i for n in range(fold_size)]

        # TODO: Generate indices of samples for the training set for the fold
        train_indices = [m for m in range(X.shape[0]) if m not in valid_indices]

        # TODO: Split the dataset into training and validation
        X_train, X_valid = [X[l] for l in train_indices], [X[l] for l in valid_indices]
        y_train, y_valid = [y[l] for l in train_indices], [y[l] for l in valid_indices]
        ...
        y_prediccion = model.predict(X_valid)
        verdad = sum(np.array(y_valid) == np.array(y_prediccion))
        accuracy_scores[i] = verdad / len(y_prediccion)
